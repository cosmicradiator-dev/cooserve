...
export function WorkerBottomNav() {
  ...
  return (
    <nav className="sticky bottom-0 z-30 w-full bg-white border-t border-slate-200 px-3 py-2 flex justify-around items-center shadow-lg">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          <Link
            ...
            className={`flex flex-col items-center gap-1 transition-colors ${
              isActive ? 'text-primary font-bold' : 'text-text-muted hover:text-slate-800'
            }`}
          >
